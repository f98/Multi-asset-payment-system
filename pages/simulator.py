...
Alice = Customer("Alice", 1000, 800,15/100 ,85/100,"Apple",170.0,10)
Bob=Customer('Bob',5000,1500,25/100,0.75,'Meta',300.0,10)
Ali=Customer('Ali',2500,1500,0.10 ,0.90,'Nike',98.0,20)
...
def outcome(value_sold, selected_customer, final_exchange_rate,increase_in_securities_value):
    if  value_sold<=4500 :
    ...
    elif value_sold>4500 : # value sold 0.7 --> shares , 0.3 in cash 
         securities_sold=selected_customer.acquisation_price*selected_customer.number_of_shares*(1+increase_in_securities_value)
         rest=value_sold- securities_sold
         if rest>0:
          total_monetary_value=((selected_customer.dollar_percentage*rest) + (selected_customer.euro_percentage * rest * (1/final_exchange_rate)))+securities_sold
...
if st.button("Apply") :
    if value_sold<=4500:
     final_amount_sold = outcome(value_sold,selected_customer,final_exchange_rate,increase_in_securities_value)
